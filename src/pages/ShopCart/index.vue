...
import throttle from 'lodash/throttle'
  export default {
    ...
    methods: {
      ...
      //修改某一产品的个数,加了节流函数的lodash包
      handler: throttle(async function (type, disNum, cart) {
        //type：区分这三个元素
        //目前disNum形参，+ 变化量 （1） - 变化量（-1） input最终个数（并不是变化量）
        //cart:那个产品（身上有id）
        //向服务器发请求修改数量
        switch (type) {
          //加号
          case "add":
            ...
            break
          case "minus":
            //判断产品的个数是大于一才能传递给服务器-1
            // if(cart.skuNum > 1) {
            //   disNum = -1
            // } else {
            //   //产品的个数小于等于1,传递给服务器的数据为零，原封不动
            //   disNum = 0
            // }
            disNum = cart.skuNum > 1 ? -1 : 0
            break
          case "change":
            //用户输入的是非法的字符，比如带有汉字，带给服务器的数字不变
    ...
    computed: {
      ...
      isAllChecked() {
        //遍历数组里面原理，只要全部元素的isChecked为1，就返回真true，只要有一个不为1，就返回false
