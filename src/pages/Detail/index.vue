        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前的路由跳转只是从a跳转到b路由，这里的加入购物车，进行路由跳转之前，发送请求把你购买的的产品
                通过请求的形式通知路由器，服务器进行储存
                -->
...
export default {
  ...
  methods: {
    ...
    changeActive(saleAttrValue, arr) {
      //排他，遍历去拿不售卖属性的属性值isChecked为零没有高亮了
      arr.forEach(item => {
        item.isChecked = '0'
      });
      //点击的那个售卖属性值
    ...
    async addShopcar() {
      //1.将产品加入到数据库（通知服务器）
      /* 2.当前这里是派发了一个action，也向服务器发送请求
        判断加入购物车成功还是失败了，进行相应的操作实质上是在干什么 
        this.$store.dispatch('addOrUpdateShopCart', { skuId: this.$route.params.skuid, skuNum:this.skuNum})
        说白了：调用仓库中的addOrUpdateShopCart函数,这个方法机上async，返回的一定是一个Promise
        要么成功，要么失败
      */
      try {
         await this.$store.dispatch('addOrUpdateShopCart', { skuId: this.$route.params.skuid, skuNum: this.skuNum })
         //3.路由跳转,产品信息带给下一级的路由
         //4.下面这种手段路由跳转一级传递参数可以
         //一些简单的数据skuNum，通过query形式给路由组件传递过去
         //产品信息的数据【比较复杂skuInfo】，通过会话存储【会话结束会消失】
         //本地存储|会话存储，一般存储的是字符串
